...
export const Component = () => {
  ...
  useEffect(() => {
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color1: { value: new THREE.Color(0x0033ff) },  // Original blue
          color2: { value: new THREE.Color(0xff0066) },  // Original magenta
          opacity: { value: 0.25 }  // Slightly reduced from 0.3 for readability
        },
        vertexShader: `
          varying vec2 vUv;
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec3 atmosphere = vec3(0.25, 0.5, 0.9) * intensity;  // Slightly dimmed from original
            
            float pulse = sin(time * 2.0) * 0.1 + 0.9;
            atmosphere *= pulse;
            
            gl_FragColor = vec4(atmosphere, intensity * 0.2);  // Slightly reduced opacity
          }
        `,
        side: THREE.BackSide,
        blending: THREE.AdditiveBlending,
  ...
  useEffect(() => {
    ...
    const validRefs = [titleRef.current, subtitleRef.current, scrollProgressRef.current].filter(Boolean);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Fade out the first hero content as user scrolls
      // Fade out within first 15% of scroll
      const fadeProgress = Math.min(scrollY / (windowHeight * 0.3), 1);
      ...
      // Define camera positions for each section - dramatic transitions
      const cameraPositions = [
        { x: 0, y: 30, z: 300 },      // Section 0 - CodeRIT - Wide view
        { x: -50, y: 60, z: 100 },    // Section 1 - & Dept of CSE - Slight pan left, rise up
        { x: 50, y: 40, z: -100 },    // Section 2 - Presents - Pan right, dive in
        { x: 0, y: 80, z: -400 },     // Section 3 - wHACKiest - Rise high, deep space
        { x: 0, y: 100, z: -800 }     // Section 4 - Final deep space
      ];
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      {/* Main content - First section: CodeRIT - fades out on scroll */}
      <div 
        ref={heroContentRef}
      ...
      <div className="scroll-sections">
        ...
        <section className="content-section" style={{ minHeight: '140vh' }}>
        ...
        {/* Section 3: Presents */}
        <section className="content-section" style={{ minHeight: '150vh' }}>
          <h1 className="hero-title">Presents</h1>
          <div className="hero-subtitle cosmos-subtitle">
            <p className="subtitle-line">A 24-Hour Hackathon</p>
            <p className="subtitle-line">Dec 12-13, 2025</p>
          </div>
        </section>
        
        {/* Section 4: wHACKiest - The Grand Finale */}
        <section className="content-section whackiest-finale" style={{ minHeight: '200vh' }}>
          <div className="whackiest-title-container">
            <TextHoverEffect text="wHACKiest" duration={0.15} />
          </div>
